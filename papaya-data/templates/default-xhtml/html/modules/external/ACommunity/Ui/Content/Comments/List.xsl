...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:import href="../Comment/Dialog.xsl"/>
  ...
  <xsl:template name="acommunity-comments-list">
    ...
    <xsl:if test="$commandName = 'reply' and $commandCommentId = $commentId">
      <xsl:call-template name="acommunity-comment-dialog">
              ...
              <div class="commentExtras">
                ...
                <xsl:if test="count(links/link[@name = 'vote_up']) &gt; 0 and count(links/link[@name = 'vote_down']) &gt; 0">
                  <div class="commentExtrasVoting">
                    <a class="commentExtrasVotingLinkVoteUp" href="{links/link[@name = 'vote_up']/text()}">[ + ]</a>
                    <a class="commentExtrasVotingLinkVoteDown" href="{links/link[@name = 'vote_down']/text()}">[ - ]</a>
                    <xsl:call-template name="float-fix" />
                  </div>
                </xsl:if>
                <div class="commentExtrasReply">
                  <xsl:if test="count(links/link[@name = 'reply']) &gt; 0">
                    <a class="commentExtrasReplyLink" href="{links/link[@name = 'reply']/text()}">Antworten</a>
                  </xsl:if>
                  <xsl:text> </xsl:text>
                </div>
              </div>
